...
TASK(UwlReceive)
{
	...
	switch(rx_packet[0]){
		case START_MANUAL:
			PWM_reset();
			PWM_set(rx_packet[3], rx_packet[4]);
		...
		case ACTUATE:
			if (sw){
				PWM_set(last_color, rx_packet[4]);
				sw=0;
			}else{ADC2ConvertedVoltage = (ADC2ConvertedValue *5000) / 65520;
				  printf("SLAVE2: sample: %d  ; %d [V]\r\n", ADC2ConvertedValue, ADC2ConvertedVoltage);
				  PWM_set(rx_packet[3], rx_packet[4]);
				  }
			last_light_val = rx_packet[4];
...
int main(void)
{
	...
	printf(" SLAVE2: Discovery board initialized!\r\n");				/* Control message */
	...
	printf("\r\nSLAVE2: Device node is ready!\r\n");					/* Control message */

	/* Demo started */
	printf("\r\nSLAVE2: Ready to recive\r\n");

	STM_EVAL_LEDOn(LED6);

	//SetRelAlarm(TaskSampleAlarm, 10, 1000);
